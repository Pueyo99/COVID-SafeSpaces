...
def arg_parse():
    ...
    parser.add_argument("--cfg", dest = 'cfgfile', help =  "Config file", default = "object_counter/config/yolov3.cfg", type = str)
    parser.add_argument("--weights", dest = 'weightsfile', help = "weightsfile", default = "object_counter/weights/yolov3.weights", type = str)
...
classes = load_classes("object_counter/data/coco.names")
...
#load the image 
    ...
    for im_num, image in enumerate(imlist[i*batch_size: min((i +  1)*batch_size, len(imlist))]):
        ...
        for o in objs:
            ...
            if "person" in o:
                num_person += 1 
        print("Hay {} personas".format(num_person))

        filename = image.split("/")[-1]
        print("Filename: ",filename)

        with open("object_counter/json_results/"+filename.replace(".jpg",".json"),"w+") as json_file:
            json.dump({"Número de personas": num_person}, json_file)
...
colors = pkl.load(open("object_counter/colors/pallete", "rb"))
